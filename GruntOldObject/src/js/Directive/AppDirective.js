    ...
    */
    ...
    .directive('ufoRate', function () {
     ...
     .directive('ufoMondyformat', function () {
         return {
             restrict: "E",
             template: '<span class="fontSize-1" style="margin-right: 20px; color: {{busiTypeClass}}">{{busiTypePrefix}}{{ufoMoneyText}}</span> <br /><span class="fontSize-0-9" style="color: {{CIClass}};  margin-right: 20px;" >{{busiTypeText}}</span>',
             link: function ($scope, element, attrs) {
                 //{{item.busiType==2?'+':'-'}}{{item.txAmt/10000 | currency:'':6}}万元
                 $scope.busiTypeClass = '#656565'; 
                 $scope.ufoMoneyText = "";
                 $scope.busiTypePrefix = "-";


                 $scope.CIClass = "#656565";
                 $scope.busiTypeText = "";


                 if ($scope.ufoBusitype == '100000001') {
                     $scope.busiTypeClass = '#F89327';
                     $scope.busiTypePrefix = "+";
                 };
                 if ($scope.ufoBusitype == '100000002') {
                     $scope.CIClass = "#F89327";
                 ...
                 if ($scope.ufoMoney < 10000) {
                     $scope.ufoMoneyText = $scope.ufoMoney + "元";
                 } else {

                     var arry = $scope.ufoMoney.split('.');
                     if (arry.length == 1) {
                         $scope.ufoMoneyText = $scope.ufoMoney/10000 + "万元";
                     } else {
                         if (arry[1].length == 1) {
                             var m = $scope.ufoMoney.replace(".", "");
                             $scope.ufoMoneyText = m / 100000 + "万元";
                         } else if (arry[1].length == 2) {
                             var m = $scope.ufoMoney.replace(".", "");
                             $scope.ufoMoneyText = m / 1000000 + "万元";
                         } else {
                             $scope.ufoMoneyText = $scope.ufoMoney + "元";
                         }
                     }
    ...
    .directive('ufoReflistduedate', function () {
        return {
            ...
            link: function ($scope, element, attrs) {
                var arry = $scope.ufoPrdname.split("*");
                $scope.dataText = "";

                var fun = function () {
                    if ($scope.ufoDate == null || $scope.ufoDate == "") {
                        $scope.dataText = '到期日:实际到期为准';
                    } else {
                        $scope.dataText = '到期日:' + $scope.ufoDate;
                    }
                }
